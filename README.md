...
| **Groq** | [console.groq.com/keys](https://console.groq.com/keys) | `llama-3.3-70b-versatile` | Fast free tier, CORS-friendly |
| **Gemini** | [aistudio.google.com/apikey](https://aistudio.google.com/apikey) | `gemini-2.0-flash` | Free tier, CORS-friendly |
| **xAI Grok** | [console.x.ai](https://console.x.ai) | `grok-4` | Uses `/api/llm` CORS proxy |
| **DeepSeek** | [platform.deepseek.com](https://platform.deepseek.com) | `deepseek-chat` | Uses `/api/llm` CORS proxy |
...
> The core build loop calls **your** provider. Keys live in this browser (or a gitignored `.env`). Providers that block CORS (xAI, DeepSeek, OpenAI) go through the same-origin `/api/llm` proxy — the key is in the request body, not stored on the server.

## CORS proxy

`api/llm.js` (Vercel) and the Vite `llmProxy()` middleware (local `npm run dev`) expose `POST /api/llm`. The browser sends `{ baseUrl, apiKey, model, messages }`. Only allow-listed hosts are forwarded. Deploy to Vercel so production builds get the proxy; local dev already has it.
...
- **Vercel** — `vercel` (auto-detects Vite + `/api/llm`). Import the GitHub repo at vercel.com/new.
